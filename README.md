...
8. **Prompt Readout** — Prompt execution is a short in-band chain:
   ...
   - Readout firmware routes prompt affinity to the nearest community header by Hamming distance over the affinity region, then reads the modal label from members stamped with that community ID.
   - Readout firmware resolves prompt heads.
   - `vm.Machine.Prompt` only accepts resolved Values from the prompt chain it injected.
...
10. **Telemetry bridge:**
   - During **🔁 `vm.Machine.Load`**, each recruitment drain sweep republishes raw resident **Value** frames (`[128]uint64` words) into **`pkg/telemetry.Bridge`**, so `SELECTED` Values with newly stamped communities are visible even when they are not returned as READY work.
...
- **Program region**: Sixteen resident instruction slots. Boolean instructions are packed 64-bit words for the **universal bitwise** path; geometric instructions occupy raw PGA opcode slots (`0x10`, `0x20`, `0x30`). **Authoring** does not hand-edit raw words: programs are written as bracket/feed source under `cmd/cfg/config.yml`, lowered by `pkg/compute/program`, and copied into this region by `primitive.Value.InstallFirmware`. When Values encounter each other, their programs run — no external interpreter needed.
...
`Context`, `Gradient`, and `Signals` are each 512-bit regions, so each holds one `pkg/core/numeric/geometry.Multivector` without changing the 1024-byte `Value` stride. The resident program stream preserves the full geometric opcode word before falling back to the Boolean low nibble, so geometric opcodes cannot collapse to `FALSE`. CPU executes raw geometric slots inside `HypercubeGossip`; Metal and CUDA expose the same PGA lane through their direct `GeometricFrame` kernels while resident mixed Boolean/PGA programs route to CPU until their gossip paths carry the raw-slot decoder. CPU uses hand-written ARM64 and AMD64 assembly for the PGA product, CUDA executes the lane as native `float64`, and Metal preserves the 64-bit frame ABI while converting to native `float32` arithmetic at the GPU boundary because Apple Metal does not expose double precision in shader code.

Each newly minted `Value` derives a stable `primitive.FrameMultivector` from its payload and writes it into `Context`. Boolean code can still inspect the same lanes through `ContextVector`, but the geometric path treats the region as a continuous coordinate. The feed compiler accepts `geometric compose`, `geometric sandwich`, and `geometric reverse`; `ProgramIR` emits the same raw slots with `OpGeometricCompose`, `OpGeometricSandwich`, and `OpGeometricReverse`. A `Value` can therefore carry its rotor and target into the compute substrate instead of relying on an external interpreter.
