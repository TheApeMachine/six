...
*/
func (dataset *BabiQADataset) Generate() iter.Seq[byte] {
	return func(yield func(byte) bool) {
		samples, err := dataset.Samples()
		if err != nil {
			errnie.Error(err, "repo", dataset.base.repo, "subset", dataset.base.subset)
			return
		}

		for _, sample := range samples {
			for _, b := range []byte(sample.Full) {
				if !yield(b) {
					return
				}
			}
...
*/
func (dataset *BabiQADataset) GeneratePrompts() iter.Seq[data.Prompt] {
...
func (dataset *BabiQADataset) load() error {
	...
	reader, err := dataset.base.downloadShard(shard, branch)
	if err != nil {
		return err
	}

	if strings.HasSuffix(shard, ".parquet") {
		return dataset.loadParquet(reader)
	}

	return dataset.loadJSON(reader)
}
...
*/
func (dataset *BabiQADataset) loadParquet(reader io.Reader) error {
...
*/
func (dataset *BabiQADataset) loadJSON(reader io.Reader) error {
	data, err := io.ReadAll(reader)
	if err != nil {
		return fmt.Errorf("huggingface: read shard: %w", err)
	}
	...
	for {
		...
		if err := dec.Decode(&row); err != nil {
			if err != io.EOF {
				errnie.Error(err, "msg", "bAbI JSON decode failure", "row", rowCount)
			}
			if err == io.EOF {
				break
			}
