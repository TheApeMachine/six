		...
		runArm := func(gated bool) []RelCantEntry {
			...
			for _, prompt := range prompts {
				...
				for step := range maxChains {
					...
					if gated {
						cant := relativeCantileverProbe(queryFP, cspans, sm.Substrate, numeric.NBasis, ratioThreshold)
					...
					for d := range 8 {
						...
						for idx := range cspans {
							...
							if gated && cspans[idx].spanLen > maxSafe {
								continue
							}
							...
							candidates = append(candidates, cand{idx, ovl, newToks, s, adjSim,
								cspans[idx].eigenPhase, cspans[idx].spanLen})
							seen[idx] = true
						}
					}
					if len(candidates) == 0 {
					...
					for _, c := range candidates {
						...
						chain = append(chain, RelCantStep{
							...
							SourceFunc: srcFn, SpanLen: c.spanLen, MaxSafe: maxSafe,
				...
				entries = append(entries, RelCantEntry{
					...
					BridgeCount: bridgeCount, Gated: gated, Chain: chain,
				})
			}
			return entries
		}
		...
		Convey("When running control and ratio-gated arms", func() {
			...
			Convey("Artifacts should be written to the paper directory", func() {
				...
				So(WriteBarChart(xAxis, []projector.BarSeries{
					...
					{Name: "Rel-Gated", Data: gatedData},
				}, "Relative Cantilever Scale Selection",
					"Tokens generated per prompt, control vs ratio-gated.",
