...
// CorrLenSplitResult holds one partition's result from the correlation length experiment.
type CorrLenSplitResult struct {
	Name          string  `json:"name"`
	Block0Size    int     `json:"block0_size"`
	Block1Size    int     `json:"block1_size"`
	Overlap       int     `json:"overlap"`
	BestGain      float64 `json:"best_gain"`
	SingleCeiling float64 `json:"single_ceiling"`
	Delta         float64 `json:"delta"`
	SuperAdditive bool    `json:"super_additive"`
	BestA1        float64 `json:"best_a1"`
	BestA2        float64 `json:"best_a2"`
	BestC         string  `json:"best_c"`
	CorrLenRatio  float64 `json:"corr_len_ratio"`
}

// CorrLenResult aggregates all partition results from the correlation length experiment.
type CorrLenResult struct {
	Splits []CorrLenSplitResult `json:"splits"`
}

// BoundaryScoreResult holds one candidate boundary's scores.
type BoundaryScoreResult struct {
	Boundary int     `json:"boundary"`
	SBalance float64 `json:"s_balance"`
	KLeft    float64 `json:"k_left"`
	KRight   float64 `json:"k_right"`
	Combined float64 `json:"combined"`
}

// GapTestResult holds one gap partition's result.
type GapTestResult struct {
	GapSize       int     `json:"gap_size"`
	Block0End     int     `json:"block0_end"`
	Block1Start   int     `json:"block1_start"`
	BestGain      float64 `json:"best_gain"`
	Delta         float64 `json:"delta"`
	SuperAdditive bool    `json:"super_additive"`
	BestA1        float64 `json:"best_a1"`
...
// AdaptiveSplitResult holds the adaptive split experiment results.
type AdaptiveSplitResult struct {
	SelectedBoundary int                   `json:"selected_boundary"`
	TopBoundaries    []BoundaryScoreResult `json:"top_boundaries"`
	AdaptiveGain     float64               `json:"adaptive_gain"`
	AdaptiveDelta    float64               `json:"adaptive_delta"`
	AdaptiveSA       bool                  `json:"adaptive_sa"`
	AdaptiveA1       float64               `json:"adaptive_a1"`
	AdaptiveA2       float64               `json:"adaptive_a2"`
	AdaptiveC        string                `json:"adaptive_c"`
	ReferenceGain    float64               `json:"reference_gain"`
	ReferenceDelta   float64               `json:"reference_delta"`
	Ceiling          float64               `json:"ceiling"`
	GapTests         []GapTestResult       `json:"gap_tests"`
}

// SteerBoundary holds steerability for one candidate boundary.
type SteerBoundary struct {
	Boundary   int     `json:"boundary"`
	SteerLeft  float64 `json:"steer_left"`
	SteerRight float64 `json:"steer_right"`
	SteerMin   float64 `json:"steer_min"`
}

// SteerValidation holds steerability + gain for a validated split.
type SteerValidation struct {
	Name          string  `json:"name"`
	Boundary      int     `json:"boundary"`
	SteerLeft     float64 `json:"steer_left"`
	SteerRight    float64 `json:"steer_right"`
...
// SteerResult holds the steerability experiment results.
type SteerResult struct {
	SelectedBoundary int               `json:"selected_boundary"`
	TopBoundaries    []SteerBoundary   `json:"top_boundaries"`
	Validations      []SteerValidation `json:"validations"`
	GapSteerLeft     float64           `json:"gap_steer_left"`
	GapSteerRight    float64           `json:"gap_steer_right"`
	Ceiling          float64           `json:"ceiling"`
...
type ValidationReport struct {
	...
	CorrLenData       CorrLenResult
	AdaptiveSplitData AdaptiveSplitResult
	SteerData         SteerResult
}
