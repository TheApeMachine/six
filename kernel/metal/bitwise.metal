...
kernel void bitwise_best_fill(
    ...
    device const IcosahedralManifold& candidate = dictionary[id];
    constant IcosahedralManifold& ctx = active_context[0];
    // Expected reality acts as teleological anchor (can be ignored if null on host side)
    constant IcosahedralManifold& expected = expected_reality[0];
