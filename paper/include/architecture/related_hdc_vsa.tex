...
addressing) provides positional binding.  The geometric opcode lane adds
a second VSA-like operator family: PGA motors compose transitions and
apply them to target coordinates without leaving the fixed frame.
...
deterministic, the sixteen Boolean operations are closed over the binary
frame, and the geometric lane is closed over the eight-lane multivector
representation stored in Context, Gradient, and Signals.
