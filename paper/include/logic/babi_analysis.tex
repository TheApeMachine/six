\subsection{bAbI QA Task 1: Single Supporting Fact}
\label{sec:babi_benchmark}

\paragraph{Task Description.}
The bAbI QA benchmark (Task~1) evaluates single-supporting-fact
question answering. Each sample consists of a short story describing
entity movements between locations, followed by a question of the
form \textit{''Where is Entity?''}. The correct
answer is the last location the entity moved to.

\paragraph{Test Conditions.}
The experiment evaluated 500 samples from the
\texttt{facebook/babi\_qa} dataset (subset \texttt{en-10k-qa1}).
Reasoning was performed using TransitiveResonance-validated
entity tracking: the system extracts the entity name from the
question, identifies its last movement sentence in the story,
and emits the location word. TransitiveResonance computes a
geometric validation score measuring structural plausibility
of the answer chord against the hypothesis residue.

\paragraph{Results.}
The system achieved an exact-match accuracy of 0.0000\%
across all 500 samples, with a mean partial score of
0.000.
A total of 500 samples failed exact match.
Table~\ref{tab:babi_failures} lists the first 20 failure cases.

\paragraph{Per-Entity Analysis.}
\textbf{Daniel}: 0/127 correct (0.0000\%).
\textbf{John}: 0/130 correct (0.0000\%).
\textbf{Mary}: 0/120 correct (0.0000\%).
\textbf{Sandra}: 0/123 correct (0.0000\%).

\paragraph{Per-Location Analysis.}
\textbf{bathroom}: 0/97 correct (0.0000\%).
\textbf{bedroom}: 0/81 correct (0.0000\%).
\textbf{garden}: 0/79 correct (0.0000\%).
\textbf{hallway}: 0/84 correct (0.0000\%).
\textbf{kitchen}: 0/89 correct (0.0000\%).
\textbf{office}: 0/70 correct (0.0000\%).
\begin{table}[h]
\centering
\caption{bAbI Task~1 failure cases (first 20).
Exact-match accuracy: 0.0000\%.
$N=500$, subset: \texttt{en-10k-qa1}.}
\label{tab:babi_failures}
\begin{tabular}{|r|l|l|l|}
\hline
\textbf{Idx} & \textbf{Entity} & \textbf{Expected} & \textbf{Observed} \\
\hline
0 & Mary & bathroom &  \\
\hline
1 & Daniel & hallway &  \\
\hline
2 & Daniel & hallway &  \\
\hline
3 & Daniel & office &  \\
\hline
4 & Sandra & bathroom &  \\
\hline
5 & Sandra & bathroom &  \\
\hline
6 & Sandra & bathroom &  \\
\hline
7 & Sandra & bathroom &  \\
\hline
8 & John & office &  \\
\hline
9 & Daniel & hallway &  \\
\hline
10 & John & bathroom &  \\
\hline
11 & Mary & bedroom &  \\
\hline
12 & John & bathroom &  \\
\hline
13 & John & bathroom &  \\
\hline
14 & Sandra & bedroom &  \\
\hline
15 & Daniel & bedroom &  \\
\hline
16 & Sandra & kitchen &  \\
\hline
17 & Sandra & hallway &  \\
\hline
18 & Sandra & hallway &  \\
\hline
19 & Daniel & hallway &  \\
\hline
\end{tabular}
\end{table}
