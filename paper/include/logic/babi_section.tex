...
The system achieved an exact-match accuracy of 96.0\%
across all 50 samples, with a mean partial score of
0.960 and an overall weighted score of
0.960.
...
\begin{table}[htbp]
  \centering
  \caption{bAbI Task~1 failure cases (showing first 2 of
    2). $N = 50$, exact accuracy
    96.0\%.}
  ...
  \begin{tabular}{rlll}
    ...
    \midrule
    35 & Mary & \texttt{kitchen} & \texttt{itchen} \\
    49 & Daniel & \texttt{kitchen} & \texttt{athroom} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[htbp]
  ...
  \caption{Trial Outcome Map for bAbI Task~1 ($N = 50$ samples).
