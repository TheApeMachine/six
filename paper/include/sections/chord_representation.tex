...
coprime and $D = 257$ is prime, distinct byte values receive distinct activation
patterns (verified by exhaustive enumeration over all $256$ values).  The
resulting value is extremely sparse: $\|\mathbf{c}(b)\|_1 = 5$ out of
$D = 257$ bits, yielding a sparsity ratio of $5/257 \approx 1.95\%$.

\paragraph{Why Coprime Multipliers.}
The choice of coprime multipliers is not arbitrary.  Multiplying by a value
$p$ and reducing modulo a prime $D = 257$ generates a permutation of the
residues $\{0, \ldots, D-1\}$ for any $p \not\equiv 0 \pmod{D}$.
Since all five multipliers are themselves prime and less than 257, each
independently generates a distinct full-period permutation,
maximizing the minimum Hamming distance between any two byte values.
...
\texttt{uint64}, yielding $D/w = 8$ operations per value):
...
For GPU dispatch, the sparse 257-bit Value is converted to a \emph{FlatValue}:
