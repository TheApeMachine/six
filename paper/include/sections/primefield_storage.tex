...
\begin{itemize}[nosep]
  \item \textbf{Insertion:}  Tokens are merged into the active manifold
        via \texttt{ValueOR} at a block index determined by the token's
        topological events and value dynamics.
...
carries signal (non-empty values), the PrimeField freezes the current
...
per-block prototype attractors.  Incoming values are compared against stored
prototypes; if similarity exceeds a threshold, the value is ``snapped'' to
the prototype---reinforcing established patterns.  Novel values that diverge
