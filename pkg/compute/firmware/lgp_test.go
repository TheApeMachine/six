...
func TestMain(m *testing.M) {
	...
	if err := core.LoadValueConfig(); err != nil {
		fmt.Fprintf(os.Stderr, "firmware_test: core.LoadValueConfig: %v\n", err)
		os.Exit(1)
	}
...
func TestIsIntron(t *testing.T) {
	Convey("Intron detection", t, func() {
		...
		Convey("Identity A with same src and dst is an intron", func() {
			instr := MakeIntron(uint16(core.Cfg.R0))
			So(IsIntron(instr), ShouldBeTrue)
		})

		Convey("XOR gate is not an intron", func() {
			instr := uint32(0x6) | (uint32(core.Cfg.R0) << 4) | (uint32(core.Cfg.R1) << 18)
...
func TestTraceEffective(t *testing.T) {
	Convey("Given a program that writes to r6", t, func() {
		var c [128]uint64
		r0 := uint16(core.Cfg.R0)
		r6 := uint16(core.Cfg.R6)
		start := int(PayloadProgramPCStart())
