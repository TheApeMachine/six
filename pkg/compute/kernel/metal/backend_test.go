...
func TestHypercubeGossipRejectsReducers(t *testing.T) {
	Convey("Given a strict program with a named reducer", t, func() {
		_, err := program.Compile(context.Background(), `
program zipf {
  set A.properties.program_id <- zipf_select(B.properties.program_id, B.properties.confidence, A.properties.temperature)
}
`, program.Layout{})

		Convey("It should reject the reducer before any backend sees it", func() {
