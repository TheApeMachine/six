...
func main() {
	...
	maxPC := int(q)
	stateIndex := viper.GetInt("value.region.state.index")
	stateAccum := viper.GetInt("value.region.state.accumulator")
	execStatusWord := stateAccum + 1
	tokenBits := viper.GetUint64("value.region.tokens.bits")
	tokensStart := viper.GetInt("value.region.tokens.start")
	tokenWords := int((tokenBits + 63) / 64)

	if stateIndex == 0 || tokenBits == 0 {
		fmt.Printf("gen primitives: value.region.state.index and value.region.tokens.bits must be non-zero (got stateIndex=%d tokenBits=%d)\n",
			stateIndex, tokenBits)
		os.Exit(1)
	}

	if tokensStart < 0 || tokensStart >= words {
		fmt.Printf("gen primitives: value.region.tokens.start must be in [0, value.words) (got tokensStart=%d words=%d)\n",
			tokensStart, words)
		os.Exit(1)
	}

	if tokensStart+tokenWords > words {
		fmt.Printf("gen primitives: token region exceeds frame: tokensStart=%d tokenWords=%d words=%d tokenBits=%d\n",
			tokensStart, tokenWords, words, tokenBits)
		os.Exit(1)
	}
	...
	writeMacro(&content, "PAYLOAD_PROGRAM_WORD_OFFSET", 4)
	writeMacro(&content, "USER_PROGRAM_PC_START", 8)
	writeMacro(&content, "MAX_PC", maxPC)
	writeMacro(&content, "TOKENS_START_WORD", tokensStart)
	writeMacro(&content, "TOKEN_WORDS", tokenWords)
	writeMacro(&content, "STATE_INDEX_WORD", stateIndex)
	writeMacro(&content, "STATE_ACCUM_WORD", stateAccum)
