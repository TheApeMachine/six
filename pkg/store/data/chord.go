...
var (
	/*
		canonicalChordBasis is a 5-mark Golomb ruler projected into GF(257).
		The pairwise deltas are unique, giving each lexical chord a sparse
		resonant fingerprint rather than a handful of arbitrary coprime hits.
	*/
	canonicalChordBasis = [5]int{0, 1, 4, 9, 11}

	/*
		byteChordMultipliers stores 3^(byte+1) mod 257 for every byte.
		Affine scaling by a primitive-root orbit preserves the sparse shape
		while rotating it through the Fermat field.
	*/
	byteChordMultipliers = initByteChordMultipliers()
)

func initByteChordMultipliers() [256]int {
	var multipliers [256]int
	for b := 0; b < len(multipliers); b++ {
...
func baseChordOffsets(b byte) [5]int {
	mul := byteChordMultipliers[int(b)]
	add := (int(b)*17 + 1) % 257

	var offsets [5]int
	for i, base := range canonicalChordBasis {
...
The chord is generated by an affine transform of a sparse 5-mark basis
...
func BaseChord(b byte) Chord {
	...
	for _, off := range baseChordOffsets(b) {
		chord.Set(off)
	}
