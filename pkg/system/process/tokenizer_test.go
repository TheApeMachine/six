...
func TestTokenizerServer(t *testing.T) {
	Convey("Given a TokenizerServer with natural language corpus", t, func() {
		...
		Convey("When generating over the corpus", func() {
			...
			Convey("Every token in a chunk should store its own single-byte chord", func() {
				...
				expectedChunkChord, err := data.BuildChord(chunk)

				So(err, ShouldBeNil)
				So(directServer.processChunk(ctx, 0, chunk, data.Chord{}), ShouldBeNil)

				records := directIns.snapshot()
				So(len(records), ShouldEqual, len(chunk))
				So(directServer.collector[0], ShouldHaveLength, 1)

				// Collector still gets the full chunk chord for prompt use
				expectedChunkSig := newChordSignature(expectedChunkChord)
				So(newChordSignature(directServer.collector[0][0]), ShouldResemble, expectedChunkSig)

				// Each insert stores a single-byte chord (5 bits)
				for idx, rec := range records {
					byteChord, _ := data.BuildChord([]byte{chunk[idx]})
					expectedByteSig := newChordSignature(byteChord)

					So(rec.symbol, ShouldEqual, chunk[idx])
					So(rec.position, ShouldEqual, uint32(idx))
					So(rec.active, ShouldEqual, 5)
					So(rec.signature, ShouldResemble, expectedByteSig)
				}
			})
		})
	...
	Convey("Given a TokenizerServer with delayed boundary emission", t, func() {
		...
		Convey("When generating over the sample", func() {
			...
			Convey("It should use chunk-relative positions for each byte", func() {
				So(err, ShouldBeNil)

				records := ins.snapshot()
				So(len(records), ShouldBeGreaterThan, 0)

				// Positions are byte offsets within each chunk, always starting from 0.
				for _, rec := range records {
					So(rec.position, ShouldBeLessThan, uint32(len(sample)))
				}
			})
		})
	})
...
func BenchmarkProcessChunk(b *testing.B) {
	...
	for _, size := range sizes {
		...
		b.Run(fmt.Sprintf("bytes_%d", size), func(b *testing.B) {
			...
			for range b.N {
				if err := server.processChunk(ctx, 0, chunk, data.Chord{}); err != nil {
					b.Fatal(err)
				}
			}
		})
