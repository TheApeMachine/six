...
/*
failWriteRW fails Write for edge Copy error coverage.
*/
type failWriteRW struct{}

func (failWriteRW) Read(p []byte) (n int, err error) {
	return 0, io.EOF
}

func (failWriteRW) Write(p []byte) (n int, err error) {
	return 0, io.ErrClosedPipe
}

func (failWriteRW) Close() error {
	return nil
}
...
func TestGraph(t *testing.T) {
	...
	Convey("Given an edge whose target rejects writes", t, func() {
		reg := new(memRW)
		src := new(memRW)
		...
		graph := NewGraph(
			WithRegistry(reg),
			WithNode(&Node{ID: "a", Component: src}),
			WithNode(&Node{ID: "b", Component: failWriteRW{}}),
			WithEdge(&Edge{From: "a", To: "b"}),
		)

		Convey("Read should wrap io.Copy errors from the edge", func() {
			_, err := graph.Write([]byte("reg"))
			So(err, ShouldBeNil)
			...
			So(n, ShouldEqual, 0)
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldContainSubstring, "graph.Read")
			So(err.Error(), ShouldContainSubstring, "closed pipe")
		})
	})
}
