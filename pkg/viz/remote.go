...
// It does not connect until the first Publish or an explicit Connect call.
...
func (r *RemoteBus) connectLocked() error {
	...
	conn, err := websocket.Dial(r.url, "", "http://localhost")
	if err != nil {
		return err
	}
	r.conn = conn
...
func (r *RemoteBus) Forward(ev Event) {
	...
	if r.conn == nil {
		if err := r.connectLocked(); err != nil {
			// Buffer up to 4096 events while disconnected.
			if len(r.buf) < 4096 {
...
// remote viz server. Returns a cleanup function. This is the glue that
// makes test-process events appear in the viz UI.
