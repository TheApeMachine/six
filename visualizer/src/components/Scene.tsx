...
/*
keysForRecentSequences builds row keys from node id, seq text, and a per-seq
occurrence counter so we never use .map index as a React key (duplicates are unique).
*/
function keysForRecentSequences(
	nodeId: string,
	sequences: readonly string[],
): { key: string; seq: string }[] {
	const seen = new Map<string, number>();
	const out: { key: string; seq: string }[] = [];
	for (const seq of sequences) {
		const ordinal = seen.get(seq) ?? 0;
		seen.set(seq, ordinal + 1);
		out.push({
			seq,
			key: `${nodeId}\u241e${ordinal}\u241e${seq}`,
		});
	}
	return out;
}
...
/*
keysForBeamHypotheses assigns stable keys from hypothesis fields plus a duplicate
ordinal so we never use .map index as a React key.
*/
function keysForBeamHypotheses(
	hypotheses: BeamState["hypotheses"],
): { key: string; hyp: BeamState["hypotheses"][number] }[] {
	const seen = new Map<string, number>();
	const out: { key: string; hyp: BeamState["hypotheses"][number] }[] = [];
	for (const hyp of hypotheses) {
		const base = `${hyp.tokens}\u241e${hyp.score}\u241e${hyp.origin}`;
		const ordinal = seen.get(base) ?? 0;
		seen.set(base, ordinal + 1);
		out.push({ hyp, key: `${base}\u241e${ordinal}` });
	}
	return out;
}

/*
keysForTrieGraphEdges builds keys from from/to/token plus a duplicate ordinal
for parallel edges with identical labels.
*/
function keysForTrieGraphEdges(
	edges: TrieGraphEdge[],
): { key: string; edge: TrieGraphEdge }[] {
	const seen = new Map<string, number>();
	const out: { key: string; edge: TrieGraphEdge }[] = [];
	for (const edge of edges) {
		const base = `${edge.from}\u241e${edge.to}\u241e${edge.token}`;
		const ordinal = seen.get(base) ?? 0;
		seen.set(base, ordinal + 1);
		out.push({ edge, key: `${base}\u241e${ordinal}` });
	}
	return out;
}
...
export default function Scene() {
	...
	const [inspectTarget, setInspectTarget] = useState<InspectorTarget | null>(
		null,
	);
		...
		const cbs: EngineCallbacks = {
			onEvent: (_ev: VizEvent) => {
				...
				if (tgt && engineRef.current) {
					...
					if (tgt.kind === "trie")
						setTrieData(eng.getTrieState(tgt.id, tgt.trieIndex ?? 0));
				}
			},
			onInspect,
			onStats: (s) => setStats(s),
			onLog: (segments) =>
				setLogs((prev) => [{ segments, ts: Date.now() }, ...prev].slice(0, 32)),
		...
		<div className="relative w-full h-screen bg-[#0a0e1a] overflow-hidden font-mono text-[#c8d6e5]">
			...
			<div className="absolute top-0 left-0 right-0 z-10 flex items-center justify-between px-3 py-1.5 bg-[#0a0e1a]/80 border-b border-[#1b2a3a] backdrop-blur-sm">
				<div className="flex items-center gap-3">
					<h1 className="text-[12px] text-[#4cc9f0] tracking-widest">
						SIX — OBSERVABILITY
					</h1>
					<span
						className={`w-2 h-2 rounded-full ${connected ? "bg-[#76ff03]" : "bg-[#f44336]"} animate-pulse`}
					/>
					<span className="text-[9px] opacity-40">
						{connected ? "ws:6600" : "disconnected"}
					</span>
				</div>
				...
				{stats && (
					<div className="flex items-center gap-4 text-[9px] opacity-60">
						<span>
							nodes <b className="text-[#ffab00]">{stats.nodeCount}</b>
						</span>
						<span>
							tries <b className="text-[#e6c930]">{stats.trieCount}</b>
						</span>
						<span>
							edges <b className="text-[#4cc9f0]">{stats.edgeCount}</b>
						</span>
						<span>
							events <b className="text-[#76ff03]">{stats.eventCount}</b>
						</span>
				...
				<div className="flex items-center gap-2">
					<button
						...
						className="text-[9px] px-2 py-0.5 border border-[#1b3a5a] hover:bg-[#1b3a5a]/50 transition-colors"
					>
					...
					<button
						...
						className={`text-[9px] px-2 py-0.5 border transition-colors ${paused ? "border-[#ffab00] text-[#ffab00]" : "border-[#1b3a5a] hover:bg-[#1b3a5a]/50"}`}
					>
						{paused ? "\u25B6 play" : "\u23F8 pause"}
					</button>
				</div>
			...
			<div className="absolute bottom-0 left-0 right-0 z-10 h-5 bg-[#0a0e1a]/90 border-t border-[#1b2a3a] flex items-center px-2 gap-2">
				<span className="text-[8px] opacity-40 w-16">
					{timelineCursor}/{timelineTotal}
				</span>
				<button
					type="button"
					className="flex-1 h-1 bg-[#1b2a3a] rounded-full relative cursor-pointer border-0 p-0"
					onClick={(e) => {
				...
				>
					<div
						className="h-full bg-[#4cc9f0]/50 rounded-full transition-all"
						style={{
							width:
								timelineTotal > 0
									? `${(timelineCursor / timelineTotal) * 100}%`
									: "0%",
						}}
					/>
				</button>
				{paused && (
					<span className="text-[8px] text-[#ffab00] animate-pulse">
						PAUSED
					</span>
				)}
			</div>
			...
			{showLog && (
				<div className="absolute bottom-6 left-3 z-10 text-[9px] bg-[#0a0e1a]/85 border border-[#1b2a3a] p-2 min-w-[340px] max-h-[200px] overflow-y-auto backdrop-blur-sm">
					...
					<div className="flex flex-col gap-0.5 text-[#5a8aaa]">
						...
						{logs.length === 0 && (
							<div className="opacity-30">waiting for events...</div>
						)}
					</div>
				</div>
			)}
			...
			{showPrompt && (
				<div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-30 bg-[#0a0e1a]/95 border border-[#2a4a6a] p-4 min-w-[500px] backdrop-blur-md shadow-2xl">
					<div className="text-[11px] text-[#4cc9f0] mb-2 tracking-wider">
						Send Prompt
					</div>
					<input
						ref={promptInputRef}
					...
					<div className="flex justify-end gap-2 mt-2">
						<button
							...
							className="text-[9px] px-3 py-1 border border-[#1b3a5a] hover:bg-[#1b3a5a]/50"
						>
							cancel
						</button>
						<button
							...
							className="text-[9px] px-3 py-1 border border-[#4cc9f0] text-[#4cc9f0] hover:bg-[#4cc9f0]/10"
						>
							send
						</button>
					</div>
			...
			{aluData && aluData.totalDispatches > 0 && (
				<div className="absolute top-12 left-3 z-10 text-[10px] bg-[#0a0e1a]/90 border border-[#1b2a3a] p-2.5 min-w-[220px] backdrop-blur-sm shadow-lg shadow-black/50">
					<h3 className="text-[#26c6da] text-[11px] mb-1 tracking-wider">
						ALU Pipeline
					</h3>
					<div className="flex justify-between">
						<span>total dispatches</span>
						<span className="text-[#76ff03]">{aluData.totalDispatches}</span>
					</div>
						...
						{Object.entries(aluData.substrates).map(([name, sub]) => (
							<div key={name} className="mb-1">
								...
								<div className="flex justify-between text-[9px] opacity-60">
									...
									<span>
										{(sub.emaDurationNs / 1000).toFixed(1)}
										{"\u00B5"}s EMA
									</span>
								</div>
								<div className="h-0.5 bg-[#1b2a3a] mt-0.5 rounded-full overflow-hidden">
									<div
										className="h-full bg-[#26c6da]/40 transition-all"
										style={{
											width: `${Math.min(100, (sub.inflight / 8) * 100)}%`,
										}}
									/>
								</div>
							</div>
					...
					{aluData.recentOps.length > 0 && (
						<div className="mt-1 border-t border-[#1b2a3a] pt-1">
							...
							{aluData.recentOps
								...
								.map((op, i) => (
									<div
										key={`${op.substrate}-${op.opcode}-${op.durationNs}-${i}`}
										className="text-[8px] flex gap-1 opacity-70"
									>
										<span
											className={
												op.substrate === "cpu"
										...
										<span className="text-[#76ff03]">
											{(op.durationNs / 1000).toFixed(1)}
											{"\u00B5"}s
										</span>
									</div>
								))}
			...
			{inspectTarget && (
				<div className="absolute top-10 right-3 z-20 bg-[#0a0e1a]/95 border border-[#2a4a6a] p-3 min-w-[320px] max-w-[400px] max-h-[calc(100vh-80px)] overflow-y-auto backdrop-blur-md shadow-2xl shadow-black/80 text-[11px]">
					...
					<h2 className="text-[#ffab00] text-[14px] tracking-wider mb-2">
						...
						{inspectTarget.kind === "trie" &&
							`${inspectTarget.id} / T${inspectTarget.trieIndex}`}
						...
						<div className="space-y-2">
								...
								<Section title="field pressure">
									{Object.entries(nodeData.pressure).map(([k, v]) => (
										<Row
											key={k}
											k={k}
											v={typeof v === "number" ? v.toFixed(6) : String(v)}
										/>
									))}
								</Section>
							)}
							...
							{Object.keys(nodeData.latencies).length > 0 && (
								<Section title="peer latencies">
									{Object.entries(nodeData.latencies).map(([peer, ms]) => (
										<Row
											key={peer}
											k={peer.substring(0, 16)}
											v={`${ms.toFixed(1)}ms`}
											color={
												ms > 50 ? "#c04040" : ms > 10 ? "#c08030" : "#408060"
											}
										/>
									))}
								</Section>
							...
							{Object.keys(nodeData.labelCounts).length > 0 && (
								<Section title="label distribution">
									{Object.entries(nodeData.labelCounts)
										.sort((a, b) => b[1] - a[1])
										.map(([label, count]) => {
											const total = Object.values(nodeData.labelCounts).reduce(
												(s, v) => s + v,
												0,
											);
											...
											return (
												<div key={label}>
													...
													<div className="h-0.5 bg-[#1b2a3a] rounded-full overflow-hidden mt-0.5">
														<div
															className="h-full bg-[#7050a0] transition-all"
															style={{ width: `${pct}%` }}
														/>
													</div>
												</div>
							...
							{nodeData.recentSequences.length > 0 && (
								<Section title="recent sequences">
									{keysForRecentSequences(nodeData.id, nodeData.recentSequences)
										.reverse()
										.slice(0, 8)
										.map(({ key, seq }) => (
											<div
												key={key}
												className="text-[10px] text-[#5a8aaa] py-0.5 border-b border-[#1b2a3a]/30 break-all"
											>
												{seq}
											</div>
										))}
							...
							{beamData && (beamData.activeCount > 0 || beamData.converged) && (
								<Section title="beam search">
									<Row
										k="active hypotheses"
										v={String(beamData.activeCount)}
										color="#408060"
									/>
									<Row
										k="rejected"
										v={String(beamData.rejectedCount)}
										color="#c04040"
									/>
									<Row
										k="best score"
										v={beamData.bestScore.toFixed(4)}
										color="#c08030"
									/>
									{beamData.converged && (
										<Row
											k="converged"
											v={beamData.lastSequence || "yes"}
											color="#76ff03"
										/>
									)}
									{beamData.hypotheses.length > 0 && (
										<div className="mt-1">
											<div className="text-[9px] opacity-40 mb-0.5">
												hypotheses
											</div>
											{keysForBeamHypotheses(
												beamData.hypotheses.slice(0, 6),
											).map(({ key, hyp }) => (
												<div
													key={key}
													className="text-[9px] flex justify-between py-0.5 border-b border-[#1b2a3a]/20"
												>
													<span className="text-[#c8d6e5] break-all max-w-[200px]">
														{hyp.tokens}
													</span>
													<span
														className={
									...
									<div className="mt-1 h-1 bg-[#1b2a3a] rounded-full overflow-hidden">
										<div
											className="h-full bg-linear-to-r from-[#ff6e40] to-[#ffab00] transition-all duration-300"
											style={{
												width: `${Math.min(100, Math.max(5, ((beamData.bestScore + 15) / 15) * 100))}%`,
											}}
										/>
									</div>
								</Section>
					...
					{inspectTarget.kind === "trie" && trieData && (
						<div className="space-y-2">
							<Section title="trie state">
								...
								<Row
									k="surprisal"
									v={trieData.surprisal.toFixed(4)}
									color={
										trieData.surprisal > 5
											? "#c04040"
											: trieData.surprisal > 2
												? "#c08030"
												: "#408060"
									}
								/>
								...
								<Row
									k="growth"
									v={`${trieData.growth >= 0 ? "+" : ""}${trieData.growth.toFixed(4)}`}
									color={trieData.growth > 0 ? "#408060" : "#c04060"}
								/>
								<Row
									k="decay mul"
									v={`x${trieData.decayMul.toFixed(3)}`}
									color="#c04060"
								/>
								<Row
									k="learn mul"
									v={`x${trieData.learnMul.toFixed(3)}`}
									color="#408060"
								/>
								<Row
									k="aligned"
									v={trieData.aligned ? "yes" : "no"}
									color={trieData.aligned ? "#c08030" : "#3a5878"}
								/>
							</Section>

							{inspectTarget.vertexVid !== undefined &&
								trieData.graphPayload && (
									<Section title="selected vertex">
										{(() => {
											const v = trieData.graphPayload.vertices.find(
												(vt) => vt.vid === inspectTarget.vertexVid,
											);
											if (!v)
												return (
													<div className="text-[#3a5878]">
											...
											return (
												<>
													...
													{trieData.graphPayload.edges.filter(
														(e) => e.to === v.vid,
													).length > 0 && (
														<div className="mt-1">
													...
													{trieData.graphPayload.truncated && (
														<div className="text-[#c08030] text-[9px] mt-1">
															Snapshot truncated server-side
														</div>
													)}
												</>
											);
					...
					{inspectTarget.kind === "pipeline" && pipeData && (
						<div className="space-y-2">
							<Section title="overview">
								...
								{pipeData.bytesProcessed > 0 && (
									<Row
										k="bytes"
										v={`${(pipeData.bytesProcessed / 1024).toFixed(1)} KiB`}
									/>
								)}
								{pipeData.inflight > 0 && (
									<Row k="inflight" v={String(pipeData.inflight)} />
								)}
								{pipeData.emaDurationMs > 0 && (
									<Row
										k="EMA duration"
										v={`${pipeData.emaDurationMs.toFixed(2)}ms`}
									/>
								)}
							</Section>
							...
							{pipeData.recentOps.length > 0 && (
								<Section title="recent operations">
									{keysForRecentSequences(
										pipeData.id,
										pipeData.recentOps.slice(-8).reverse(),
									).map(({ key, seq: op }) => (
										<div
											key={key}
											className="text-[10px] text-[#5a8aaa] py-0.5 break-all"
										>
											{op}
										</div>
									))}
									...
									{(() => {
										...
										return (
											<>
												...
												<Row
													k="last"
													v={`${(sub.lastDurationNs / 1000).toFixed(1)}\u00B5s`}
												/>
												<Row
													k="EMA"
													v={`${(sub.emaDurationNs / 1000).toFixed(1)}\u00B5s`}
												/>
												<div className="h-1 bg-[#1b2a3a] rounded-full overflow-hidden mt-1">
													<div
														className="h-full bg-[#4a80c0] transition-all"
														style={{
															width: `${Math.min(100, (sub.inflight / 8) * 100)}%`,
														}}
													/>
						...
						<div className="space-y-2">
							<Section title="algorithm component">
								...
								<div className="text-[9px] text-[#5a8aaa] mt-1">
									Click a DHT node to inspect its algorithm stack state. This
									component runs inside each node's algo.Stack.
								</div>
							</Section>
						</div>
					)}
...
function Section({
	...
	return (
		<div className="border-b border-[#1b2a3a] pb-1.5">
			<h4 className="text-[10px] text-[#5a7a9a] tracking-wider mb-0.5">
				{title}
			</h4>
			{children}
		</div>
