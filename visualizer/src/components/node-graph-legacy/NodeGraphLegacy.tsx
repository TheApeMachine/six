...
import type { TimeSliderHandle } from "../node-graph/components/TimeSlider";
import {
	createGraphGeometry,
	disposeGeometry,
	type GeometryResult,
...
import { NodeGraphLegacyLayoutControls } from "./NodeGraphLegacyLayoutControls.tsx";
import { NodeGraphLegacyTimeControls } from "./NodeGraphLegacyTimeControls.tsx";
...
function extractNodeMetrics(graph: Graph): NodeMetrics[] {
	...
	for (const name of nodeNames) {
		...
		const brightnessNorm = typeof data?.brightness_norm === 'number' ? data.brightness_norm : 0.5;
		const weightMagNorm = typeof data?.weight_mag_norm === 'number' ? data.weight_mag_norm : 0.5;
...
export function NodeGraphLegacy({
	graph: externalGraph,
	layout = "force",
	...
	const lastLayoutBoundsRef = useRef<{ center: THREE.Vector3; radius: number } | null>(null);
	const lastCameraSaveMsRef = useRef<number>(0);
	const backgroundTextureRef = useRef<THREE.Texture | null>(null);

	const { fitCameraToGraph, restoreCameraState, saveCameraState, setOrbitTargetToLayoutCenter } =
		useMemo(
			() =>
				Camera({
					cameraRef,
...
export default NodeGraphLegacy;
