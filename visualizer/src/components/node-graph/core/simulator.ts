...
export class Simulator {
  ...
  constructor(renderer: THREE.WebGLRenderer) {
    ...
    this.passThruShader = new THREE.ShaderMaterial({
      ...
      fragmentShader: /* glsl */ `
        ...
        void main() {
          vec2 uv = gl_FragCoord.xy / vec2(NODESWIDTH, NODESWIDTH);
          gl_FragColor = texture2D(tInput, uv);
        }
      `.replace(/NODESWIDTH/g, '1.0'),
    })
  ...
  init(config: SimulatorConfig): void {
    this.config = config
    const { nodesAndEdges, nodesAndEpochs, nodesWidth, edgesWidth, epochsWidth, epochOffset } = config
    ...
    this.passThruShader.fragmentShader = /* glsl */ `
      uniform sampler2D tInput;
      void main() {
        vec2 uv = gl_FragCoord.xy / vec2(${nodesWidthStr}, ${nodesWidthStr});
        gl_FragColor = texture2D(tInput, uv);
      }
    `
    ...
    this.velocityUniforms.edgeIndices.value = generateIndicesTexture(nodesAndEdges, nodesWidth)
    this.velocityUniforms.edgeData.value = generateDataTexture(nodesAndEdges, edgesWidth)
    this.velocityUniforms.layoutPositions.value = generateZeroedPositionTexture(nodesAndEdges, edgesWidth)

    // Set up node attrib shader textures
    this.nodeAttribUniforms.epochsIndices.value = generateIndicesTexture(nodesAndEpochs, nodesWidth)
    this.nodeAttribUniforms.epochsData.value = generateEpochDataTexture(nodesAndEpochs, epochsWidth, epochOffset)
    this.nodeAttribUniforms.edgeIndices.value = this.velocityUniforms.edgeIndices.value
    this.nodeAttribUniforms.edgeData.value = this.velocityUniforms.edgeData.value
    this.nodeAttribUniforms.nodeIDMappings.value = generateIdMappings(nodesAndEpochs, nodesWidth)
  ...
  private renderTexture(input: THREE.Texture | THREE.WebGLRenderTarget, output: THREE.WebGLRenderTarget): void {
    this.mesh.material = this.passThruShader
    this.passThruShader.uniforms.tInput.value = input instanceof THREE.WebGLRenderTarget ? input.texture : input
   ...
   */
  simulate(delta: number, temperature: number, epochMin: number, epochMax: number): void {
    if (!this.rtPosition1 || !this.rtPosition2 || !this.rtVelocity1 || !this.rtVelocity2 ||
        !this.rtNodeAttrib1 || !this.rtNodeAttrib2) {
      return
    }

    if (this.flipflop) {
      if (temperature > 0.1) {
        this.renderVelocity(this.rtPosition1, this.rtVelocity1, this.rtVelocity2, delta, temperature)
        this.renderPosition(this.rtPosition1, this.rtVelocity2, this.rtPosition2, delta)
      }
      this.renderNodeAttrib(this.rtNodeAttrib1, this.rtNodeAttrib2, epochMin, epochMax, delta)
    } else {
      if (temperature > 0.1) {
        this.renderVelocity(this.rtPosition2, this.rtVelocity2, this.rtVelocity1, delta, temperature)
        this.renderPosition(this.rtPosition2, this.rtVelocity1, this.rtPosition1, delta)
      }
      this.renderNodeAttrib(this.rtNodeAttrib2, this.rtNodeAttrib1, epochMin, epochMax, delta)
    }
