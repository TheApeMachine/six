 ...
 * - Node rendering with texture-based positioning
 * - Edge rendering between nodes
 * - Force-directed layout simulation (velocity and position)
 * - Node attribute animation (size, opacity)
...
// Uses the same varyings as nodeVertexShader but ignores most of them
...
// Matches original behavior from analytics-master
...
void main() {
    ...
    if (hoverMode > 0.0) {
        ...
        // If hovering over a real node
        if (selectedNode >= 0.0) {
            // If you are the selected node or a neighbor
            if (neighborPixel > 0.0 || selfPixel > 0.0) {
                selfAttrib.y = 0.8; // Light up self or neighbors
            ...
            // Not hovering over anything - show time-based nodes
            if (epochPixel > 0.0) {
                selfAttrib.x = 600.0;
                selfAttrib.y = 0.8;
            }
        ...
        // Black out the rest of the scene
        if (selfAttrib.y > 0.0) {
            selfAttrib.y -= delta * 2.5;
        }
        ...
        if (selectedNode >= 0.0) {
            // If you are the selected node or a neighbor
            if (neighborPixel > 0.0 || selfPixel > 0.0) {
                selfAttrib.y = 0.3; // Light up self or neighbors dimly
    ...
    gl_FragColor = vec4(selfAttrib.xy, 0.0, 0.0);
}
`

// Text/label vertex shader
// Matches original vs-text.glsl behavior
...
void main() {
    ...
    gl_Position = projectionMatrix * (
        ...
        vec4(labelPositions.xy * 0.3, 0.0, 0.0)
    );
}
`

// Text/label fragment shader
// Matches original fs-text.glsl behavior
