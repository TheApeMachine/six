...
function worldLayoutPoint(key: string, extent: number, inset: number): { x: number; y: number } {
...
function communitySpiralPos(seq: number, cx: number, cy: number): { x: number; y: number } {
...
export function initEngine(container: HTMLDivElement, callbacks: VizCallbacks) {
  ...
  canvas.addEventListener("click", (e) => {
    if (Math.abs(e.clientX - panStartX) > 4 || Math.abs(e.clientY - panStartY) > 4) return;
  ...
  const eventLog: { ts: number; kind: string; detail: string; color: string }[] = [];
  ...
  const beam: BeamEngineState = {
    activeCount: 0, bestScore: 0, converged: false, lastSequence: "",
    collectCount: 0, composeCount: 0, breakCount: 0, convergeCount: 0,
  };
  ...
  function applyEvent(ev: VizEvent) {
    ...
    // ── Peer events ─────────────────────────────────────────────────────────
      ...
      addLog("system", `peer+ ${ev.src}→${ev.tgt} bkt=${ev.vals?.bucket ?? "?"}`);
    }
    else if (ev.kind === EK.PeerRemoved) {
      addLog("system", `peer- ${ev.src}→${ev.tgt}`);
    }
    else if (ev.kind === EK.PeerLatency) {
      addLog("system", `lat ${ev.src}→${ev.tgt} ${(ev.vals?.latency_ms ?? 0).toFixed(1)}ms`);
    }

    // ── Data flow ───────────────────────────────────────────────────────────
    else if (ev.kind === EK.ValuePublished) {
      addLog("system", `pub ${ev.src} key=${ev.meta?.key?.substring(0, 12) ?? "?"}`);
    }
    ...
    // ── Field dynamics ──────────────────────────────────────────────────────
      ...
      addLog("field", `digest ${ev.src} S=${surprisal.toFixed(3)} H=${entropy.toFixed(3)} g=${growth.toFixed(3)}`);
    }
    else if (ev.kind === EK.FieldPressure) {
      const decay = ev.vals?.decay ?? 0;
      const learn = ev.vals?.learning ?? 0;
      const prune = ev.vals?.prune ?? 0;
      addLog("field", `pressure ${ev.src} d=${decay.toFixed(3)} l=${learn.toFixed(3)} p=${prune.toFixed(3)}`);
    }
    else if (ev.kind === EK.EigenmodeDetected) {
      const modeCount = ev.vals?.mode_count ?? 0;
      const dominantEnergy = ev.vals?.dominant_energy ?? 0;
      addLog("eigenmode", `${ev.src} modes=${modeCount} energy=${dominantEnergy.toFixed(3)}`);
    }
    ...
    // ── Trie / value-graph instrumentation ─────────────────────────────────
      ...
      addLog("trie", `decay ${ev.src} factor=${(ev.vals?.factor ?? 0).toFixed(4)}`);
    }
    else if (ev.kind === EK.TriePrune) {
      addLog("trie", `prune ${ev.src} removed=${ev.vals?.removed ?? "?"}`);
    }
    else if (ev.kind === EK.TriePredict) {
      addLog("trie", `predict ${ev.src} score=${(ev.vals?.score ?? 0).toFixed(4)}`);
    }
      ...
      addLog("trie", `signal ${ev.src} S=${surprisal.toFixed(3)} H=${entropy.toFixed(3)}`);
    }
    else if (ev.kind === EK.TrieCoupling) {
      addLog("trie", `coupling ${ev.src}↔${ev.tgt} str=${(ev.vals?.strength ?? 0).toFixed(3)}`);
    }
    else if (ev.kind === EK.TrieMode) {
      addLog("trie", `mode ${ev.src} m=${ev.vals?.mode ?? "?"}`);
    }
    else if (ev.kind === EK.TriePressure) {
      addLog("trie", `pressure ${ev.src} d=${(ev.vals?.decay ?? 0).toFixed(3)} l=${(ev.vals?.learn ?? 0).toFixed(3)}`);
    }

    // ── Pool ────────────────────────────────────────────────────────────────
    else if (ev.kind === EK.PoolSchedule) {
      poolScheduled++;
      addLog("pool", `schedule inf=${ev.vals?.inflight ?? "?"} q=${ev.vals?.queued ?? "?"}`);
    }
    else if (ev.kind === EK.PoolComplete) {
      poolCompleted++;
      const dur = ev.vals?.duration_ns ? `${(ev.vals.duration_ns / 1e6).toFixed(2)}ms` : "";
      addLog("pool", `complete ${dur} inf=${ev.vals?.inflight ?? "?"}`);
    }
    ...
    // ── Beam search ─────────────────────────────────────────────────────────
      ...
      beam.collectCount++;
      const active = ev.vals?.active ?? 0;
      beam.activeCount = Math.max(beam.activeCount, active);
      addLog("beam", `collect ${ev.src} active=${active} hyp=${ev.vals?.hypotheses ?? "?"}`);
    }
      ...
      addLog("beam", `compose ${ev.src} score=${score.toFixed(4)} ${seq.substring(0, 20)}`);
    }
    else if (ev.kind === EK.BeamBreak) {
      beam.breakCount++;
      addLog("beam", `BREAK ${ev.src} reason=${ev.meta?.reason || ev.lbl || "?"}`);
    }
      ...
      const seq = ev.meta?.sequence || ev.meta?.tokens || "";
      if (seq) beam.lastSequence = seq.substring(0, 40);
      addLog("beam", `CONVERGE ${ev.src} score=${score.toFixed(4)} ${seq.substring(0, 24)}`);
    }

    // ── Compiler → ALU → Finalizer ──────────────────────────────────────────
    else if (ev.kind === EK.CompilerCompile) {
      compilerTotal++;
      compilerLastProg = ev.lbl || ev.meta?.program || "";
      addLog("compiler", `compile prog="${compilerLastProg.substring(0, 20)}" ${ev.src}`);
    }
    else if (ev.kind === EK.ALUDispatch) {
      aluTotal++;
      const sub = ev.meta?.substrate || ev.src || "cpu";
      const durNs = ev.vals?.duration_ns ?? 0;
      const existing = aluSubs.get(sub) ?? { inflight: 0, total: 0, lastDurNs: 0 };
      existing.total++;
    ...
    // ── Prompt ──────────────────────────────────────────────────────────────
      ...
      valueMap.set(vid, {
        ...
        resonance: 1, gap: 1, resolved: false,
        age: 0, prevId: "", nextId: "",
        telemetry: tel,
        actionResonance: 0,
        memberIndex: 0,
    ...
    // ── Ingest pipeline ─────────────────────────────────────────────────────
      ...
      valueMap.set(vid, {
        ...
        resonance: 0.5, gap: 1, resolved: false,
        age: 0, prevId: "", nextId: "",
        telemetry: tel,
        actionResonance: 0,
        memberIndex: 0,
    ...
    // ── Orchestrator / Community ────────────────────────────────────────────
      ...
      if (c) {
        ...
        if (v) {
          v.communityId = cid;
          v.resonance = 1;
          v.memberIndex = c.memberIds.size - 1;
          v.telemetry = snapshotTelemetry(ev);
      ...
      valueMap.set(vid, {
        ...
        label: prog, content: "",
        resonance: 1, gap: 0, resolved: false,
        age: 0, prevId: "", nextId: "",
        telemetry: tel,
        actionResonance: ar,
        memberIndex: 0,
      ...
      valueMap.set(vid, {
        ...
        label: prog, content: "",
        resonance: 0.8, gap: 0, resolved: false,
        age: 0, prevId: "", nextId: "",
        telemetry: tel,
        actionResonance: 0,
        memberIndex: 0,
    ...
    // ── Belief gap ──────────────────────────────────────────────────────────
      ...
      addLog("emission", `#${cid} members=${memberCount} conc=${concentration.toFixed(3)}`);
    }
  ...
  function connect() {
    ...
    ws.onmessage = (msg) => {
      ...
      try {
        ...
        addLog("error", `malformed frame: ${msgText}${preview ? ` bytes=${preview}` : ""}`);
      }
    };
  ...
  function drawCommunities() {
    for (const [, c] of communityMap) {
      ...
      const radius = Math.max(60, maxDist + 30) + Math.sin(frameCount * 0.06) * 4;
      ...
      if (c.memberIds.size >= 2) {
        ...
        ctx.fillText(
          ...
          c.center.x, c.center.y + radius + fontSize + 4,
        );
        ...
        if (c.actionCount > 0 || c.reactionCount > 0) {
          ...
          ctx.fillText(
            ...
            c.center.x, c.center.y + radius + fontSize * 2 + 8,
          );
        }
        ...
        if (c.saturated) {
          ...
          ctx.fillText(
            ...
            c.center.x, c.center.y + radius + fontSize * 3 + 12,
          );
        }
      }
    }
  ...
  function drawHUD() {
    ...
    ctx.fillText(`  data: ${data}  resolved: ${resolvedVals}`, x, y); y += 14;
    ...
    ctx.fillText(`bootstrap: ${bootstrapNodeIds.length} peers   json: ${stats.wireJsonBlobs}`, x, y); y += 14;

    ctx.fillStyle = "rgba(255,255,255,0.25)";
    ctx.fillText(`zoom: ${camZoom.toFixed(2)}x${autoFitEnabled ? "  (auto)" : ""}`, x, y); y += 20;

    ctx.fillStyle = "rgba(255,255,255,0.3)";
    ctx.font = "9px monospace";
    ctx.fillText("drag: pan  |  scroll: zoom  |  dblclick: re-fit  |  click: inspect", x, y); y += 16;
  ...
  */
  function drawSubsystemPanel() {
    ...
    if (beam.collectCount > 0 || beam.composeCount > 0) {
      ctx.fillStyle = beam.converged ? "rgba(0,255,150,0.6)" : "rgba(80,200,255,0.5)";
      ctx.fillText(
      ...
      ctx.fillText(`BEAM  collect:${beam.collectCount}  compose:${beam.composeCount}`, px, py);
      py -= 16;
    }
    ...
    if (aluSubs.size > 0) {
      for (const [sub, state] of aluSubs) {
        ...
        const barFill = Math.min(pw - 60, Math.round((state.inflight / Math.max(1, state.total)) * (pw - 60)));
        ctx.fillStyle = "rgba(255,140,60,0.15)";
        ctx.fillRect(px + 60, py - 8, barW > 0 ? Math.min(barW, pw - 60) : pw - 60, 8);
        ctx.fillStyle = "rgba(255,140,60,0.5)";
        ctx.fillRect(px + 60, py - 8, barFill, 8);
        ctx.fillStyle = "rgba(255,140,60,0.55)";
        const ms = state.lastDurNs > 0 ? `${(state.lastDurNs / 1e6).toFixed(2)}ms` : "";
    ...
    if (totalSent > 0 || totalRecv > 0) {
      ...
      ctx.fillText(`sent:${totalSent}  recv:${totalRecv}  nodes:${gossipSent.size}`, px, py);
      py -= 12;
  ...
  function emitGraphSnapshot() {
    ...
    for (const [, community] of communityMap) {
      if (community.memberIds.size === 0) continue;
  ...
  return {
    ...
    sendPrompt(text: string) {
      ...
      fetch(`http://${host}:${port}/api/prompt`, {
        ...
        addLog("error", `prompt POST failed — is viz server running at ${host}:${port}?`);
      });
    },
  };
}
