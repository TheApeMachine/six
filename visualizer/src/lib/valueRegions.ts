...
import {
	...
	INSTR_OPCODE_SHIFT,
	INSTR_POP_END_SHIFT,
	INSTR_PRED_BIT_SHIFT,
	INSTR_PRED_COND_SHIFT,
	INSTR_PRED_START_SHIFT,
	INSTR_SPAN_MASK,
	INSTR_START_MASK,
	INSTR_SRC_A_FROM_B_SHIFT,
	INSTR_STAGE_SHIFT,
...
export function decodeInstructionWord(word: bigint): DecodedInstruction {
	return {
		...
		dstStart: Number(
			(word >> BigInt(INSTR_DST_START_SHIFT)) & INSTR_START_MASK,
		),
		bSpan: Number((word >> BigInt(INSTR_B_SPAN_SHIFT)) & INSTR_SPAN_MASK) + 1,
		bStart: Number((word >> BigInt(INSTR_B_START_SHIFT)) & INSTR_START_MASK),
		...
		predCond: Number((word >> BigInt(INSTR_PRED_COND_SHIFT)) & 0x7n),
		aInd: 0,
		bType: 0,
		predicate: Number((word >> BigInt(INSTR_PRED_BIT_SHIFT)) & 0x1n),
		emit: Number((word >> BigInt(INSTR_EMIT_SHIFT)) & 0x1n),
		srcAFromB: Number(
			(word >> BigInt(INSTR_SRC_A_FROM_B_SHIFT)) & 0x1n,
		),
		stage: Number((word >> BigInt(INSTR_STAGE_SHIFT)) & 0x1n),
		popEnd: Number((word >> BigInt(INSTR_POP_END_SHIFT)) & 0x1n),
	};
}
